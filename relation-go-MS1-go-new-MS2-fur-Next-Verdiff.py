...
for key in myorgdict.keys():
    ...
    while idx < 57:
            if 'downsample' in mpltllist[idx]:
                for atv in torch.nonzero(mydict[key][mpltllist[idx]]): 
                    dikt=sorted(Counter(torch.flatten(torch.topk(torch.mul(eval(reallist[idx]).weight[int(atv)],torch.reshape(mydict[key][mpltllist[idx-4]],(int(mydict[key][mpltllist[idx-4]].size(dim=0)),1,1))),5,0)[1]).tolist()).items(),key=lambda item: item[1],reverse=True)
                    print(key,reallist[idx]+"'s Kernel","#"+str(int(atv[0])),"related to",reallist[idx-4]+"'s kernels:#",dikt)
                idx+=2
            else:
                for atv in torch.nonzero(mydict[key][mpltllist[idx]]): 
                    dikt=sorted(Counter(torch.flatten(torch.topk(torch.mul(eval(reallist[idx]).weight[int(atv)],torch.reshape(mydict[key][mpltllist[idx-1]],(int(mydict[key][mpltllist[idx-1]].size(dim=0)),1,1))),5,0)[1]).tolist()).items(),key=lambda item: item[1],reverse=True)
